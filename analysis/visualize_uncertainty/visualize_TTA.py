...
tta_prediction_path = os.path.join(folder_path, f'pid_{patient_id}_tta_{tta_idx:02d}.npy')
...
norm = mcolors.Normalize(vmin=0, vmax=umap_max)
...
with open(tta_prediction_path, 'rb') as f:
    tta_pred = np.load(f)
...
vis = VisualizeUncertaintyMapV2(
    image, y_true, tta_pred, uncertainty_map, intersection, union)
vis.start()

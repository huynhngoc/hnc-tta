...
if group1_threshold_index is not None:
    ax1.axvline(x=num_tta_cols[group1_threshold_index], color='red', linestyle='--', label=f'Difference threshold = {threshold}, consecutive iterations = {num_iterations}')
...
ax1.set_title(f'Group 1: Mean Cross-DSC for \n 2 TTA-predictions < Mean Cross-DSC for 5 TTA-predictions')
...
if group2_threshold_index is not None:
    ax2.axvline(x=num_tta_cols[group2_threshold_index], color='red', linestyle='--', label=f'Difference threshold = {threshold}, consecutive iterations = {num_iterations}')
...
ax2.set_title(f'Group 2: Mean Cross-DSC for \n 2 TTA-predictions >= Mean Cross-DSC for 5 TTA-predictions')
...
# Dividing patients into groups based on the slope between mean cross dice value for 2 and 5 TTA-predictions
group1 = MAASTRO_df[MAASTRO_df['mean_dice_02'] < MAASTRO_df['mean_dice_07']]
group2 = MAASTRO_df[MAASTRO_df['mean_dice_02'] >= MAASTRO_df['mean_dice_07']]
...
ax1.set_ylabel('Mean Cross-DSC')
ax1.set_title(f'Group 1: Mean Cross-DSC for \n 2 TTA-predictions < Mean Cross-DSC for 7 TTA-predictions')
ax1.legend(loc='lower left')
ax1.set_xticks(num_tta_cols)
ax1.set_xticklabels(new_x_labels)
ax1.set_ylim(0.0, 1.0)

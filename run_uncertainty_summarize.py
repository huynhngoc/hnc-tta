...
if __name__ == '__main__':
    ...
    for pid in ous_df.patient_idx:
        ...
        for i in range(1, iter+1):
            with open(args.source + f'/results/{args.name}/OUS/' + str(pid) + f'/{i:02d}.npy', 'rb') as f:
                print(f"Appending prob.map from file: {args.source}/results/{args.name}/OUS/{str(pid)}/{i:02d}.npy")
    ...
    for pid in maastro_df.patient_idx:
        ...
        y_pred = []
        for i in range(1, iter+1):
            with open(args.source + f'/results/{args.name}/MAASTRO/' + str(pid) + f'/{i:02d}.npy', 'rb') as f:
                y_pred.append(np.load(f))
        y_pred = np.stack(y_pred, axis=0).mean(axis=0)
        uncertainty_map = - y_pred * np.log(y_pred)
        true_positive = (y_pred > 0.5).astype(float) * y_true
